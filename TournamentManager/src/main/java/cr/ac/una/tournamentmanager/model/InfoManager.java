...
public class InfoManager {
    ...
    public void saveSports() throws IOException{
     ...
     try(FileWriter writer = new FileWriter("Sports.json")){
         gson.toJson((ArrayList<SportDto>) AppContext.getInstance().get("fullSportArrayList"), writer);
         writer.flush();
    ...
    public void SaveTournaments(){
        ...
        try(FileWriter writer = new FileWriter("Tournament.json")){
            gson.toJson((ArrayList<TourneyDto>) AppContext.getInstance().get("FullTournamentsArrayList"));
            writer.flush();
           Mensaje message = new Mensaje();
